...
has api_url => ( is => 'ro', default => 'https://nekos.life/api/v2/img/lizard' );
    ...
    $self->ua->get_p($url)->then(sub
        {
            ...
            {
                my $error = { 'code' => $tx->res->code, 'error' => 'Could not retrieve a random lizard from the Lizard API' };
                $promise->resolve($error);
                return $promise;
            }
            if ( $tx->res->code == 200 and !defined $tx->res->json->{'url'} )
            {
                my $error = { 'code' => 404, 'error' => 'Lizard API returned OK but did not include an image URL' };
                $promise->resolve($error);
                return $promise;
            }
